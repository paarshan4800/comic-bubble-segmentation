...
image = cv2.imread('./sample_images/steve.jpeg')
#original = image.copy()
...
for i in range(len(contours)):
	...
	if cv2.imwrite("panel{}.jpg".format(i),panel):
		print('image saved')

	
	"""
	plt.imshow(panel, cmap = 'gray', interpolation = 'bicubic')
	plt.xticks([]), plt.yticks([])  
	plt.show()
	plt.clf()
	"""

	#plt.savefig("panel{}.jpg".format(i))
